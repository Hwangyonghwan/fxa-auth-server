...
module.exports = (
    ...
    return P.all(promises)
      .spread((devices, redisSessionTokens = {}) => {
        ...
        return devices.map(device => {
          return mergeDeviceInfoFromRedis(uid, device, redisSessionTokens, lastAccessTimeEnabled)
        })
      })
      .catch(err =>{
        if (isNotFoundError(err)) {
  ...
  function mergeDeviceInfoFromRedis(uid, device, redisSessionTokens, lastAccessTimeEnabled) {
